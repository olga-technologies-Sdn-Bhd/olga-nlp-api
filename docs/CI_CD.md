...
The PostgreSQL server has public network access disabled. It uses the delegated subnet `snet-postgresql` and private DNS zone `private.postgres.database.azure.com`. The Container Apps environment uses `snet-container-apps` for VNet integration. The NLP API has external HTTPS ingress so clients and developers can reach the API and Swagger from a browser. The worker has no ingress.

The development Swagger UI is available at `https://ca-olga-nlp-api-dev.agreeableocean-8bb4ca77.malaysiawest.azurecontainerapps.io/swagger`. External ingress exposes the entire API, not only Swagger. All endpoints remain unauthenticated during the temporary MVP phase, so do not deploy sensitive or production data until authentication and authorization are enforced.
...
The API image listens on port `8080`. Terraform is the sole owner of Container Apps ingress, its external exposure, HTTPS policy, target port, traffic routing, and probes. Use `/health` as the process liveness endpoint and `/ready` as the readiness endpoint; `/ready` verifies PostgreSQL connectivity. The application workflow updates image digests and runtime application settings but does not mutate ingress. The worker has no ingress and must have at least one always-on replica so queued work is continuously consumed. Published API and worker images use immutable commit tags and deployment resolves both to their digests. The workflow waits for the API revision to become healthy and for the worker revision to reach the running state.
